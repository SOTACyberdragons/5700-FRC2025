...
public class VisionMoveToTargetRight extends Command {
  ...
  public VisionMoveToTargetRight(CommandSwerveDrivetrain drivetrain, VisionSubsystem visionsubsystem,
  ...
  @Override
  public void execute() {
    // if(visionsubsystem.getTargetVisibleLL1()){
    // // Retrieve alignment commands from the vision subsystem
    //   forwardCommand = visionsubsystem.getForward();
    //   lateralCommand = visionsubsystem.getLateral();
    //   rotationCommand = visionsubsystem.getRotation();

    //   forwardCommand = forwardPID.calculate(forwardCommand, 0.7); // Target forward distance (1 meter away) 0.477
    //   lateralCommand = lateralPID.calculate(lateralCommand);
    //   rotationCommand = rotationPID.calculate(rotationCommand, 0);


    //     // Apply the vision alignment command to the drivetrain

    //   } else{
    //     forwardCommand = forwardSupplier.get();
    //     lateralCommand = lateralSupplier.get();
    //     rotationCommand = rotationSupplier.get();
    //   }
    ...
    if(visionsubsystem.getTargetVisibleLL1() || visionsubsystem.getTargetVisibleLL2()){
      ...
      forwardCommand = forwardPID.calculate(visionsubsystem.getForward2(), 0.33); // Target forward distance (1 meter away) 0.477
      lateralCommand = lateralPID.calculate(visionsubsystem.getLateral2(),0.020);
      rotationCommand = rotationPID.calculate(visionsubsystem.getRotation2(), 0.0);

      }else if(visionsubsystem.getTA2()<visionsubsystem.getTA1()){

        forwardCommand = forwardPID.calculate(visionsubsystem.getForward(), 0.33); // Target forward distance (1 meter away) 0.477
        lateralCommand = lateralPID.calculate(visionsubsystem.getLateral(),0.28);
        rotationCommand = rotationPID.calculate(visionsubsystem.getRotation(), 0.0);
